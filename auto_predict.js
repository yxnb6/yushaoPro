...
function readGist() {
    return new Promise((resolve) => {
        https.get({
            ...
            path: `/gists/${GIST_ID}`,
            headers: { 'Authorization': `Bearer ${GIST_TOKEN}`, 'User-Agent': 'PC28' }
        }, (res) => {
            let data = '';
            res.on('data', c => data += c);
            res.on('end', () => {
...
function updateGist(data) {
    ...
    return new Promise((resolve, reject) => {
        const req = https.request({
            ...
            hostname: 'api.github.com',
            path: `/gists/${GIST_ID}`,
            headers: { 'Authorization': `Bearer ${GIST_TOKEN}`, 'Content-Type': 'application/json', 'User-Agent': 'PC28' }
        }, (res) => {
...
async function main() {
    ...
    try {
        ...
        const lastPred = cloudData.predictions[0];
